// 💎 SCORING ORGAN MOTION PRESETS
// Framer Motion + Math Physics
...
// PLANET (Profil-Körper)
export const planet = {
  initial: { scale: 0.92, opacity: 0 },
  animate: {
    scale: 1,
    opacity: 1,
    transition: { duration: TIME.base, ease: EASE.enter }
  }
};
...
// PARTIKEL (Text-Einspeisung)
export const particleFlow = {
  initial: { opacity: 0, y: 0 },
  animate: {
    opacity: [0, 1, 0],
    y: -120,
    transition: { duration: TIME.short, ease: EASE.drift }
  }
};
...
export const projection = {
  ...
  animate: {
    ...
    scale: 1,
    y: 0,
    transition: { duration: TIME.short, ease: EASE.enter }
  },
  exit: {
    opacity: 0,
    y: -6,
