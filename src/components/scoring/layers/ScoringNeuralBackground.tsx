...
export default function ScoringNeuralBackground() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      nodes.forEach((node, i) => {
        ...
        const pulseSize = 1.5 + Math.sin(node.energy * Math.PI * 2) * 0.5;
        ctx.beginPath();
        ctx.arc(node.x, node.y, pulseSize, 0, Math.PI * 2);
        const gradient = ctx.createRadialGradient(node.x, node.y, 0, node.x, node.y, pulseSize * 2);
        gradient.addColorStop(0, `rgba(157, 0, 255, ${0.8 + node.energy * 0.2})`);
        ...
        nodes.slice(i + 1).forEach((otherNode) => {
          ...
          if (distance < connectionDistance) {
            ...
            ctx.strokeStyle = `rgba(157, 0, 255, ${opacity * (0.4 + energyBoost * 0.4)})`;
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
        });
