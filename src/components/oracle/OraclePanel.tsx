  ...
  const loadProfileData = async (format: string) => {
    try {
      ...
      if (response.ok) {
        ...
        if (profile?.drift_thresholds) {
          Object.entries(profile.drift_thresholds).forEach(([k, v]) => {
            originals[`drift_${k}`] = v as number;
          });
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Object.entries(modifiedProperties).forEach(([id, value]) => {
        ...
        } else if (id.startsWith('drift_')) {
          driftThresholds[id.replace('drift_', '')] = value;
        }
      });
      ...
      if (Object.keys(driftThresholds).length > 0) payload.drift_thresholds = driftThresholds;
      ...
      if (response.ok) {
        const mods = Object.entries(modifiedProperties).map(([id, newValue]) => ({
          property: id.replace(/^(entity|threshold|method|drift)_/, ''),
