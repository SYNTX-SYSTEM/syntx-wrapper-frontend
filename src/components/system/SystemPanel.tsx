...
// GLASS PANEL
// ═══════════════════════════════════════════════════════════════
function GlassPanel({ children, title, icon, status, glowColor = '#00d4ff' }: {
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <h3 style={{ margin: 0, flex: 1, fontFamily: 'monospace', fontSize: 14, color: glowColor, letterSpacing: 2 }}>
          {title}
        </h3>
...
// METRIC ROW
// ═══════════════════════════════════════════════════════════════
function MetricRow({ label, value, color = '#00d4ff', icon }: {
  label: string;
  value: string | number;
  color?: string;
  icon?: string;
  ...
  return (
    <div style={{
      ...
      <span style={{ flex: 1, fontFamily: 'monospace', fontSize: 12, color: 'rgba(255,255,255,0.6)', letterSpacing: 1 }}>
        {label}
      </span>
      <span style={{ fontFamily: 'monospace', fontSize: 14, color, fontWeight: 700 }}>
        {value}
      </span>
    </div>
  );
...
// WRAPPER STATS CARD
// ═══════════════════════════════════════════════════════════════
function WrapperStatsCard({ wrapper, isDefault, onSetDefault }: { 
  wrapper: Wrapper; 
  isDefault: boolean;
  onSetDefault: () => void;
}) {
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
        ...
        <span style={{ fontFamily: 'monospace', fontSize: 14, color, fontWeight: 700, letterSpacing: 1 }}>
          {displayName}
        </span>
        {isDefault && (
          <span style={{
            padding: '3px 8px', borderRadius: 6, fontSize: 9, fontFamily: 'monospace',
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div style={{ textAlign: 'center', padding: 12, background: 'rgba(0,0,0,0.2)', borderRadius: 10 }}>
            <div style={{ fontSize: 24, fontWeight: 700, color, fontFamily: 'monospace' }}>{stats.total_requests || 0}</div>
          ...
          <div style={{ textAlign: 'center', padding: 12, background: 'rgba(0,0,0,0.2)', borderRadius: 10 }}>
            <div style={{ fontSize: 24, fontWeight: 700, color: '#10b981', fontFamily: 'monospace' }}>{stats.success_rate || 100}%</div>
      ...
      {!isDefault && (
        <button onClick={onSetDefault} style={{
          ...
          border: `1px solid ${color}40`, background: `${color}10`, color: color,
...
export default function SystemPanel() {
  ...
  const fetchData = useCallback(async () => {
    try {
      setLoading(true);
      const [healthData, configData, wrapperData] = await Promise.all([
        api.getHealth(),
        api.getConfig(),
        api.getWrappers(),
      ]);
              ...
              <>
                <div style={{
                  ...
                  <span style={{ 
                    ...
                    color: systemStatus === 'online' ? '#10b981' : '#ef4444', letterSpacing: 3,
                  }}>
            ...
            {loading ? (
              ...
              <>
                <MetricRow 
                  ...
                  value={sysConfig?.active_wrapper?.replace('syntex_wrapper_', '').toUpperCase() || 'NONE'} 
                  icon="📦" color="#f59e0b" 
                />
                <MetricRow label="TOTAL WRAPPERS" value={wrappers.length} icon="📚" color="#d946ef" />
                <MetricRow 
                  label="ACTIVE WRAPPER" 
                  value={wrappers.find(w => w.is_active)?.name.replace('syntex_wrapper_', '').toUpperCase() || 'NONE'} 
                  icon="✅" color="#10b981" 
                />
                ...
                <button onClick={fetchData} style={{
                  ...
                  color: '#00d4ff', fontFamily: 'monospace', fontSize: 12, cursor: 'pointer',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
            {wrappers.map(wrapper => (
              <WrapperStatsCard 
                key={wrapper.name} 
                wrapper={wrapper}
                isDefault={sysConfig?.active_wrapper === wrapper.name}
                onSetDefault={() => handleSetDefault(wrapper.name)}
              />
            ))}
          </div>
        </GlassPanel>
