      ...
      <div style={{ width: '100%', maxWidth: 800, maxHeight: '90vh', background: 'linear-gradient(135deg, #0a1a2e, #050b14)', borderRadius: 20, border: '1px solid rgba(0,212,255,0.3)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 24 }}>
          {loading ? (
            ...
            <pre style={{ margin: 0, padding: 16, background: 'rgba(0,0,0,0.3)', borderRadius: 12, fontSize: 12, lineHeight: 1.6, color: 'rgba(255,255,255,0.8)', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
              {content}
            </pre>
          )}
        </div>
...
export default function WrapperControl() {
  ...
  const {
    items: paginatedWrappers,
    pagination,
    sorting,
    setPage,
    setPageSize,
    toggleSort,
  } = usePagination(wrappers, 10, { key: 'name', direction: 'asc' });

  if (loading) {
    return <div style={{ padding: 40, textAlign: 'center', color: 'rgba(255,255,255,0.4)' }}>Loading Wrappers...</div>;
  }
  ...
  return (
    <div style={{
        ...
        {paginatedWrappers.map((wrapper) => (
          <div
            ...
            style={{
              ...
              background: wrapper.is_active ? 'rgba(16,185,129,0.05)' : 'transparent',
            }}
          >
            <div
            ...
            >
              ...
              <div>
                <div style={{ fontSize: 13, fontFamily: 'monospace', color: wrapper.is_active ? COLORS.green : 'white' }}>
                  {wrapper.name.replace('syntex_wrapper_', '')}
                </div>
                {wrapper.is_active && (
                  <span style={{ fontSize: 9, color: COLORS.green, fontFamily: 'monospace' }}>ACTIVE</span>
                )}
              </div>
            </div>
            <div style={{ fontSize: 11, fontFamily: 'monospace', color: 'rgba(255,255,255,0.5)', display: 'flex', alignItems: 'center' }}>
              {wrapper.size_human}
            </div>
            <div style={{ fontSize: 10, fontFamily: 'monospace', color: 'rgba(255,255,255,0.4)', display: 'flex', alignItems: 'center' }}>
              {new Date(wrapper.last_modified).toLocaleDateString('de-DE')}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {!wrapper.is_active && (
                <button
                  onClick={() => handleActivate(wrapper.name)}
                  style={{
                    padding: '6px 12px',
                    borderRadius: 6,
      ...
      {/* Modal */}
      {selectedWrapper && (
        <WrapperDetailModal wrapper={selectedWrapper} onClose={() => setSelectedWrapper(null)} />
      )}
    </div>
  );
}
